...
export default function ListaConversaciones({
    ...
    function marcarLeido(conversacion: ConversacionModelo) {
        const tieneNoLeidos = conversacion.mensajes?.some(
            (mensaje) => mensaje.emisor_id !== auth.user.id && !mensaje.leido,
        );

        if (tieneNoLeidos) {
            router.patch(
                leer(conversacion.id).url,
                {},
                {
                    preserveScroll: true,
                    only: ['conversaciones'],
    ...
    return (
        <div className="flex w-1/2 flex-col gap-2 rounded-lg border p-2 pt-2">
            {conversaciones.map((conversacion) => {
                const tieneNoLeidos = conversacion.mensajes?.some(
                    (mensaje) =>
                        mensaje.emisor_id !== auth.user.id && !mensaje.leido,
                );
                    ...
                    >
                        <div className="flex w-full flex-col gap-1">
                            ...
                            <div className="text-xs font-medium">
                                {descifrar(
                                    String(conversacion.asunto),
                                    Number(conversacion.desplazamiento_asunto),
                                    conversacion.excepciones_asunto as Record<
                                        number,
                                        string
                                    >,
                                )}
                        ...
                        <div className="line-clamp-2 text-xs text-muted-foreground">
                            {conversacion.ultimo_mensaje?.contenido
                                ? descifrar(
                                      ...
                                      conversacion.ultimo_mensaje
                                          .excepciones_contenido as Record<
                                          number,
                                          string
                                      >,
                                  )
